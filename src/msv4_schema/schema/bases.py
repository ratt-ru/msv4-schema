...
def _dataarray_new(
    ...
    **kwargs,
    ...
    # Redirect to msv4_schema.DataArray constructor
...
def xarray_dataarray_schema(cls):
    ...
    * one field called "``data``" annotated with :py:data:`~msv4_schema.schema.typing.Data`
      to indicate the array type
    * fields annotated with :py:data:`~msv4_schema.schema.typing.Coord` to indicate mappings of
      dimensions to coordinates (coordinates directly associated with dimensions
      should have the same name as the dimension)
    * fields annotated with :py:data:`~msv4_schema.schema.typing.Attr` to declare attributes

    Decorated schema classes can be used with
    :py:func:`~msv4_schema.schema.check.check_array` for checking
    ...
    For example::
        ...
        from msv4_schema.schema import xarray_dataarray_schema
        from msv4_schema.schema.typing import Data, Coord, Attr
    ...
    * For the returned :py:class:`~xarray.DataArray` object ``data``, ``coo``,
    ...
    # Make schema
    cls.__msv4_schema_array_schema = dataclass.xarray_dataclass_to_array_schema(cls)
...
def is_dataarray_schema(val: typing.Any):
    return type(val) == type and hasattr(val, "__msv4_schema_array_schema")
...
def _dataset_new(cls, *args, data_vars=None, coords=None, attrs=None, **kwargs):
    ...
    # Redirect to msv4_schema.Dataset constructor
...
def xarray_dataset_schema(cls):
    ...
    * fields annotated with :py:data:`~msv4_schema.schema.typing.Coord` to indicate mappings of
      dimensions to coordinates (coordinates directly associated with dimensions
      should have the same name as the dimension)
    * fields annotated with :py:data:`~msv4_schema.schema.typing.Data`
      to indicate data variables
    * fields annotated with :py:data:`~msv4_schema.schema.typing.Attr` to declare attributes

    Decorated schema classes can be used with
    :py:func:`~msv4_schema.schema.check.check_dataset` for checking
    ...
    # Make schema
    schema = dataclass.xarray_dataclass_to_dataset_schema(cls)
    cls.__msv4_schema_dataset_schema = schema
...
def is_dataset_schema(val: typing.Any):
    return type(val) == type and hasattr(val, "__msv4_schema_dataset_schema")
...
def dict_schema(cls):
    ...
    :py:func:`~msv4_schema.schema.check.check_dict` for checking dictionieries
    ...
    # Make schema
    cls.__msv4_schema_dict_schema = dataclass.xarray_dataclass_to_dict_schema(cls)
...
def is_dict_schema(val: typing.Any):
    return type(val) == type and hasattr(val, "__msv4_schema_dict_schema")
